...
def app(df, X, y, model_type):
    ...
    # Set the page title
    st.title("Visualise Crop Recommendation System")
    ...
    if st.checkbox("N vs P Plot"):
        sns.color_palette("rocket", as_cmap=True)
        ax = sns.scatterplot(x="N", y="P", data=df)
        st.pyplot()

    if st.checkbox("Show Sample Results"):
        unique_labels = df['label'].unique()
        label_counts = df['label'].value_counts()
        data = label_counts.values
    ...
    if st.checkbox("Plot Decision Tree"):
        ...
        dot_data = tree.export_graphviz(
            ...
            feature_names=X.columns, class_names=df['label'].unique()
        )
